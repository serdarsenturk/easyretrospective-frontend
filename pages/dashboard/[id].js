...
function Dashboard({boards}) {
  ...
  const handleClick = () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/members/${id}/boards` , {
      method: 'POST',
      headers: {
  ...
  const handleDelete = (board) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/members/${id}/boards/${board.code}` , {
      method: 'DELETE',
      headers: {
          'Content-Type': 'application/json',
      }
    ...
    return (
      ...
      <Button style={ { margin: '3rem'}} variant="primary" onClick={() => handleClick()}>CREATE BOARD</Button>
...
Dashboard.getInitialProps = async (context) => {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/members/${id}/boards`)
